...
use world::{Brush, Frame, Location, World};
use world::terrain;
use world::query;
use sprite::{Layer, Sprite};
use backend;
use view;
...
impl GameView {
    ...
    pub fn draw(&self, context: &mut backend::Context, screen_area: &Rect<f32>) {
        ...
        for (&chart_pos, origins) in chart.iter() {
            ...
            let t = query::terrain(&self.world, loc);

            let screen_pos = view::chart_to_view(chart_pos) + center;

            self.generate_sprites(loc, |layer, _, brush, frame_idx| {
                sprites.push(Sprite {
                    layer: layer,
    ...
    pub fn generate_sprites<F>(&self, loc: Location, mut draw: F)
        where F: FnMut(Layer, Angle, &Resource<Brush>, usize)
    {
        use self::Angle::*;

        let terrain = query::terrain(&self.world, loc);
        let kernel = Kernel::new(&self.world, loc);
