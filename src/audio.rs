...
impl<W: Wave, E: Envelope> Wave for Enveloped<W, E> {
    fn sample(&self, t: f64) -> f64 {
        self.wave.sample(t) * self.envelope.volume(t)
    }
}
...
impl Envelope for Adsr {
    fn volume(&self, mut t: f64) -> f64 {
        ...
        if t < self.attack_s {
            return lerp(0.0, 1.0, t / self.attack_s);
        }
        t -= self.attack_s;

        if t < self.decay_s {
            return lerp(1.0, self.sustain_volume, t / self.decay_s);
        }
        t -= self.decay_s;

        if t < sustain_s {
            return self.sustain_volume;
        }
        t -= sustain_s;
