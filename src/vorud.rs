...
static LUT: [Option<Decode>; 26] = [Some(V(0)), // a (ASCII 97)
                                    Some(C(0)), // b
                                    None, // c
                                    Some(C(1)), // d
                                    Some(V(1)), // e
...
fn vorud_chunk(data: u16) -> String {
    ...
    [CS[c1 as usize], VS[v2 as usize], CS[c3 as usize], VS[v4 as usize], CS[c5 as usize]]
        .iter()
        .map(|&x| x)
        .collect()
}
...
impl FromVorud<()> for u32 {
    fn from_vorud(v: &Vorud) -> Result<u32, ()> {
        ...
        Ok(v[3] as u32 + ((v[2] as u32) << 8) + ((v[1] as u32) << 16) + ((v[0] as u32) << 24))
    }
}
...
mod test {
    ...
    fn test_vorud() {
        ...
        assert_eq!(Ok(1234u16),
                   super::durov_chunk(&super::vorud_chunk(1234)[..]));
        assert_eq!("togas",
                   &super::vorud_chunk(super::durov_chunk("togas").unwrap())[..]);
        assert_eq!(super::Vorud("babab-babab".to_string()), 0u32.to_vorud());
        assert_eq!(super::Vorud("babab-babad".to_string()), 1u32.to_vorud());
        assert_eq!(Ok(1u32),
                   super::FromVorud::from_vorud(&super::Vorud("babab-babad".to_string())));
    }
}
