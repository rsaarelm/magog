        ...
        for cy in range(-2, 2) {
            for cx in range(-2, 2) {
                ...
                loop {
                    ...
                    // XXX: Need to cache the chunks instead of running
                    // this every time.
                    let chk = Chunk::new(*chunk).unwrap();

                    // Don't have two-region chunks on map edge, that can
                    // get unconnected regions.
                    if on_edge && !chk.connected {
                        continue;
                    }
                    break;
                }
                ...
                for (glyph, x, y) in chunk.chars().map2d() {
                    let terrain = match legend(glyph) {
                        Some(t) => t,
                        None => Void
                    };
    ...
    pub fn new(text: &str) -> Result<Chunk, String> {
        ...
        for (glyph, x, y) in text.chars().map2d() {
            ...
            cells.insert((x, y), match legend(glyph) {
                Some(t) => t,
                None => return Err(format!("Unrecognized chunk glyph {}", glyph)),
            });
        }
