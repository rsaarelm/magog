...
pub struct HexFov<F> {
    ...
    range: u32,
    stack: Vec<Sector>,
    /// Make additional corners visible in the side channel to make the FOV
    /// look better on fake-isometric maps.
...
impl<F: Fn<(V2<i32>,), Output=bool>> HexFov<F> {
    /// Create an iterator that will yield the field of view around the origin
    /// V2(0, 0) up to hex grid distance range, with cells for which is_opaque
    /// returns true blocking visibility further away in their direction.
