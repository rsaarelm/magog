...
pub struct HexFov<F, T> {
    ...
    is_wall_f: Box<Fn(Point2D<i32>, &T) -> bool>,
          ...
          T: Eq + Clone
{
    pub fn new(init: T, f: F) -> HexFov<F, T> {
        ...
        HexFov {
            ...
            is_wall_f: Box::new(move |_, _| false),
    ...
    pub fn fake_isometric<G>(mut self, is_wall_f: G) -> HexFov<F, T>
        where G: Fn(Point2D<i32>, &T) -> bool + 'static
    {
        self.is_wall_f = Box::new(is_wall_f);
        self
    }
}
            ...
            if current.pt.is_below(current.end) && current_value.is_some() {
                ...
                if let Some(side_pos) = current.pt.side_point() {
                    ...
                    if next.is_below(current.end) &&
                       (self.is_wall_f)(current.pt.to_v2(), &current.group_value) {
                        ...
                        if let Some(next_value) = (self.f)(next.to_v2(), &current.group_value) {
                            ...
                            if next_value == current.group_value &&
                               (self.is_wall_f)(next.to_v2(), &next_value) &&
                               (self.f)(side_pos, &current.group_value).is_none() &&
                               (self.is_wall_f)(side_pos, &current.group_value) {
