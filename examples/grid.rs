...
static PROJECTIONS: [(V2<f32>, V2<f32>); 3] = [(V2(16.0, 8.0), V2(-16.0, 8.0)), /* Isometric */
...
fn draw_cell(ctx: &mut Canvas, proj: &Projection, cell: V2<i32>, dim: bool) {
    ...
    // Procedural color.
    let mut color = Rgba::new(noise(n * 3) / 2.0 + 0.5,
                              noise(n * 3 + 1) / 2.0 + 0.5,
                              noise(n * 3 + 2) / 2.0 + 0.5,
                              1.0);
    if dim {
    ...
    for &p in [Anchor::TopLeft,
               Anchor::TopRight,
               Anchor::BottomRight,
               Anchor::BottomLeft]
                  .iter() {
        vertices.push(Vertex::new(proj.project(base.point(p)),
                                  0.5,
                                  tex,
                                  color,
                                  color::BLACK));
...
fn main() {
    ...
    loop {
        ...
        let proj = Projection::new(PROJECTIONS[projection].0,
                                   PROJECTIONS[projection].1)
                       .unwrap()
                       .view_offset(screen_offset);

        let mouse_rect = Rect(mouse_pos.map(|x| x as f32) - V2(160.0, 90.0),
                              V2(320.0, 180.0));
        let mouse_cell = proj.inv_project(mouse_pos.map(|x| x as f32))
                             .map(|x| x.floor() as i32);

        for pt in proj.inv_project_rectangle(&screen_rect).iter() {
            let cell = pt.map(|x| x as i32);
            if cell != mouse_cell {
                draw_cell(&mut ctx, &proj, cell, true);
            }
        }
        ...
        for pt in proj.inv_project_rectangle(&mouse_rect).iter() {
            let cell = pt.map(|x| x as i32);
            if cell != mouse_cell {
                draw_cell(&mut ctx, &proj, cell, false);
            }
        }
        ...
        for event in ctx.events().into_iter() {
            match event {
                ...
                Event::KeyPress(k) => {
                    match k {
                        Key::A => {
                            scroll_delta.0 = -1.0 * scroll_speed;
                        }
                        Key::D => {
                            scroll_delta.0 = 1.0 * scroll_speed;
                        }
                        Key::W => {
                            scroll_delta.1 = -1.0 * scroll_speed;
                        }
                        Key::S => {
                            scroll_delta.1 = 1.0 * scroll_speed;
                        }
